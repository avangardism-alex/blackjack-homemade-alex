...
export default function Card({ card, hidden = false }: { card?: TCard; hidden?: boolean }) {
  if (hidden) return <div className="w-14 h-20 rounded-lg bg-slate-700 card-shadow border border-slate-600" />;
  if (!card) return <div className="w-14 h-20" />;
  return (
    <div className="w-14 h-20 rounded-lg bg-white card-shadow border border-slate-200 flex flex-col justify-between p-1 transition-transform hover:scale-105">
